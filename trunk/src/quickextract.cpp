...
{
  rarProcess *process = new rarProcess(this, "rar", QStringList() << "x", destination, QStringList(), khistory -> currentText());
  connect(process, SIGNAL(processCompleted(bool)), this, SLOT(accept()));
  process -> start();
}
