...
akuSearchLine::akuSearchLine ( QTreeWidget *treeWidget, QWidget *parent ) : KLineEdit ( parent )
{
  tree = treeWidget;
...
{
  if(filt.size() < 3 && !text().isEmpty())
  return; // do not search if pattern is less than 3 characters long
  ...
  QList<QTreeWidgetItem*> selectedToClear = tree-> selectedItems(); //deselezioniamo gli elementi già selezionati
  for ( int h = 0; h < selectedToClear.size(); h++ )
    tree-> setItemSelected ( selectedToClear[h], false ); //deselezioniamo gli elementi già selezionati

  restoreList();
  QString tmp;
  if ( !text().isEmpty()) {
    if ( !filterBySelection )  {  //se è abilitato il filtro ad esclusione
      for ( int i = 0; i < tree-> topLevelItemCount(); i++ ) {
        tmp = ( tree-> topLevelItem ( i ) ) -> text ( 0 );
        if ( !caseSensitive ) {  //se non c'è il case sensitive
          if ( !tmp.contains( filt, Qt::CaseInsensitive )) { //era diverso
      ...
      if ( !caseSensitive )  {  //se non c'è il case sensitive...
        ...
        for ( int i = 0; i < tree-> topLevelItemCount(); i++ ) {
          tmp = ( tree-> topLevelItem ( i ) ) -> text ( 0 );
          if ( tmp.contains( filt ) ) {  //se trovo l'elemento lo seleziono
            tree->setItemSelected ( tree-> topLevelItem ( i ), true ); //lo seleziono
  ...
  if ( redSearchLine && !filt.isEmpty() ) {   //se è necessario rendiamo rossa la searchline
    QPalette palette;
    QColor alertColor;
    ...
    if (greenSearchLine && !filt.isEmpty()) {
      QPalette palette;
      QColor greenColor;
...
void akuSearchLine::restoreList()
{
  for ( int j = 0; j < filteredItems.size(); j++ ) {
    if ( tree -> isItemHidden ( filteredItems[j] ) ) tree -> setItemHidden ( filteredItems[j], false );
    if ( tree -> isItemSelected ( filteredItems[j] ) ) tree -> setItemSelected ( filteredItems[j], false );
  }
  filteredItems.clear();
}
