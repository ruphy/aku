...
akuCompressionWidget::akuCompressionWidget(QWidget *parent) : QWidget(parent)
{
   ...
   QLabel *label = new QLabel(i18n("Compression level"));
...
{
 if(comboSplit -> currentIndex() > 0 ) return comboSplit -> itemData(comboSplit -> currentIndex()).toDouble();
    else{
...
{
 if(index > 0 ) comboSplit -> setEditable(false);
   else{
