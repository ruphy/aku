...
void akuSearchLine::filter ( QString filt ) //funzione di filtro della lista basandosi su searchLine
{
  ...
  {
    //filteredItems.clear();
    if ( !filterBySelection ) //se ï¿½ abilitato il filtro ad esclusione
    {
      for ( int i = 0; i < tree-> topLevelItemCount(); i++ )
      {
        tmp = ( tree-> topLevelItem ( i ) ) -> text ( 0 );
        if ( !caseSensitive )   //se non c'Ã¨ il case sensitive
        {
  ...
  if ( redSearchLine && !filt.isEmpty() ) //se Ã¨ necessario rendiamo rossa la searchline
  {
    QPalette palette;
    QColor alertColor;
